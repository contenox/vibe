...
const (
	// ChainSessionNew runs the chain in a fresh session.
	ChainSessionNew = "new"
	// ChainSessionReused asks the machine to reuse a session across triggers; unsupported machines refuse.
...
const (
	...
	// CodeCursorEvicted answers a [Resume] whose cursor the producer no longer retains.
...
// IsControl reports whether a type is relay-level control traffic.
...
// Validate reports whether f is structurally addressable. It never rejects an unknown Type.
func (f Frame) Validate() error {
	...
	for name, v := range map[string]string{
		...
		// JSON encoding rewrites invalid UTF-8 to U+FFFD, so a routing key would not survive a hop.
