...
| **Data governance** | Local SQLite state, [environment scrubbing](/docs/guide/environment-scrubbing/), secrets resolved from env at request time, region-pinned backends ([below](#sovereign-deployment-options)), and [workspace roots](/docs/reference/contenox-cli/#contenox-workspace) bounding where sessions may operate. |
