...
| `execute_config.tools` | No | Toolset allowlist: `[]`=none, `["*"]`=every connected toolset with **no exceptions** (`native-` and `decl-` namespaced ones included), `["a","b"]`=named, `["*","!x"]`=all-except, where the exclusion wins over `*` in any order. Absent/`null`=none — the task has no tools until this field explicitly grants some. |
