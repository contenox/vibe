// Package localtools: plan_summary tools — persists the typed-JSON handover
// produced by the summarizer chain. This tools is the terminal node in the
// per-step summarizer subgraph compiled by plancompile.Compile.
//
// Two tools under one tools name:
//
...
//                   whatever ChatHistory the tools received as input. Always
//                   returns "done".
//
...
const planSummaryToolsName = "plan_summary"

// PlanSummaryTools is registered under the "plan_summary" tools name and dispatches
// to the persist/fallback tools compiled into the per-step summarizer subgraph.
type PlanSummaryTools struct {
	store planstore.Store
}

// NewPlanSummaryTools wires the tools against a planstore.Store so persist and
// fallback tools can write directly to the plan_steps table.
func NewPlanSummaryTools(store planstore.Store) taskengine.ToolsRepo {
	return &PlanSummaryTools{store: store}
}

// Exec routes to persist / fallback based on ToolsCall.ToolName.
func (h *PlanSummaryTools) Exec(ctx context.Context, startTime time.Time, input any, debug bool, toolsCall *taskengine.ToolsCall) (any, taskengine.DataType, error) {
	if toolsCall == nil {
		return nil, taskengine.DataTypeAny, errors.New("plan_summary: tools call required")
	}
...
func (h *PlanSummaryTools) persist(ctx context.Context, input any) (any, taskengine.DataType, error) {
...
// from whatever ChatHistory the tools received (compiled chain should bind its
// InputVar to the executor's terminal task so this tools sees the executor's
// full conversation, not the summarizer's).
func (h *PlanSummaryTools) fallback(ctx context.Context, input any) (any, taskengine.DataType, error) {
	_, stepID, ok := taskengine.PlanStepContextFromContext(ctx)
	if !ok || stepID == "" {
		return nil, taskengine.DataTypeAny, errors.New("plan_summary: plan/step context not set on ctx (WithPlanStepContext)")
	}
...
// directly since the tools is invoked with the executor's ChatHistory bound via
...
// Supports advertises the tools name to resolveToolsNames and the MCP/remote
// allowlist machinery. Only the tools NAME is advertised here; tool names
// ("persist", "fallback") live under ToolsCall.ToolName.
func (h *PlanSummaryTools) Supports(ctx context.Context) ([]string, error) {
	return []string{planSummaryToolsName}, nil
}

// GetSchemasForSupportedTools returns empty: this tools is invoked directly by
// the compiled plan DAG (HandleTools), not surfaced as a tool the model can
// choose via execute_tool_calls.
func (h *PlanSummaryTools) GetSchemasForSupportedTools(ctx context.Context) (map[string]*openapi3.T, error) {
	return map[string]*openapi3.T{}, nil
}

// GetToolsForToolsByName returns empty for the same reason as above.
func (h *PlanSummaryTools) GetToolsForToolsByName(ctx context.Context, name string) ([]taskengine.Tool, error) {
	return []taskengine.Tool{}, nil
}

var _ taskengine.ToolsRepo = (*PlanSummaryTools)(nil)
