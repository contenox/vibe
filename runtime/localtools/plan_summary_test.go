...
// exercises them in the test binary. These two are the only ones the tools calls.
...
func TestPlanSummary_Persist_ValidJSON(t *testing.T) {
	store := newFakeStore()
	tools := NewPlanSummaryTools(store)
	...
	out, dt, err := tools.Exec(ctx, time.Now(), string(raw), false, &taskengine.ToolsCall{Name: "plan_summary", ToolName: "persist"})
	if err != nil {
		t.Fatal(err)
	}
	if out != "ok" {
...
func TestPlanSummary_Persist_InvalidJSON_ReturnsInvalidWithoutWriting(t *testing.T) {
	...
	out, _, err := tools.Exec(ctx, time.Now(), "not json", false, &taskengine.ToolsCall{Name: "plan_summary", ToolName: "persist"})
	if err != nil {
		t.Fatal(err)
	}
	if out != "invalid" {
...
func TestPlanSummary_Persist_MissingRequiredField_ReturnsInvalid(t *testing.T) {
	...
	out, _, _ := tools.Exec(ctx, time.Now(), raw, false, &taskengine.ToolsCall{Name: "plan_summary", ToolName: "persist"})
	if out != "invalid" {
...
func TestPlanSummary_Persist_StepDoneMarkerIsStripped(t *testing.T) {
	...
	out, _, _ := tools.Exec(ctx, time.Now(), body, false, &taskengine.ToolsCall{Name: "plan_summary", ToolName: "persist"})
	if out != "ok" {
...
func TestPlanSummary_Persist_MarkerOnlyIsInvalid(t *testing.T) {
	...
	out, _, _ := tools.Exec(ctx, time.Now(), "===STEP_DONE===", false, &taskengine.ToolsCall{Name: "plan_summary", ToolName: "persist"})
	if out != "invalid" {
...
func TestPlanSummary_Fallback_AlwaysWritesAndReturnsDone(t *testing.T) {
	...
	out, _, err := tools.Exec(ctx, time.Now(), hist, false, &taskengine.ToolsCall{Name: "plan_summary", ToolName: "fallback"})
	if err != nil {
		t.Fatal(err)
	}
	if out != "done" {
...
func TestPlanSummary_Persist_NoCtx_Errors(t *testing.T) {
	...
	// No WithPlanStepContext — tools must refuse rather than write to an unknown row.
	_, _, err := tools.Exec(context.Background(), time.Now(), `{"outcome":"success","summary":"x","handover_for_next":"y"}`, false,
		&taskengine.ToolsCall{Name: "plan_summary", ToolName: "persist"})
	if err == nil {
...
func TestPlanSummary_UnknownTool_Errors(t *testing.T) {
	tools := NewPlanSummaryTools(newFakeStore())
	ctx := taskengine.WithPlanStepContext(context.Background(), "plan-1", "step-1")
	_, _, err := tools.Exec(ctx, time.Now(), "", false, &taskengine.ToolsCall{Name: "plan_summary", ToolName: "unknown"})
	if err == nil {
