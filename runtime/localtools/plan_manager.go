// Package localtools — plan_manager tools
...
const planManagerToolsName = "plan_manager"

// PlanManagerTools lets the chat LLM orchestrate plans via tool calls.
type PlanManagerTools struct {
	db              libdb.DBManager
...
func (h *PlanManagerTools) Exec(
	...
	if tools == nil {
		return nil, taskengine.DataTypeAny, fmt.Errorf("plan_manager: tools call required")
	}

	switch tools.ToolName {
	case "create_plan":
		return h.createPlan(ctx, input, tools)
	case "run_next_step":
		return h.runNextStep(ctx)
	case "get_plan_status":
		return h.getPlanStatus(ctx)
	case "retry_step":
		return h.retryStep(ctx, input, tools)
	case "skip_step":
		return h.skipStep(ctx, input, tools)
	default:
		return nil, taskengine.DataTypeAny, fmt.Errorf("plan_manager: unknown tool %q", tools.ToolName)
	}
}

// createPlan calls the planner chain and persists the new plan to SQLite.
func (h *PlanManagerTools) createPlan(ctx context.Context, input any, tools *taskengine.ToolsCall) (any, taskengine.DataType, error) {
	goal := extractGoal(input, tools)
	if goal == "" {
...
func (h *PlanManagerTools) runNextStep(ctx context.Context) (any, taskengine.DataType, error) {
...
func (h *PlanManagerTools) getPlanStatus(ctx context.Context) (any, taskengine.DataType, error) {
	st := planstore.New(h.db.WithoutTransaction(), h.workspaceID)
	activePlan, err := st.GetActivePlan(ctx)
	if err != nil {
...
func (h *PlanManagerTools) retryStep(ctx context.Context, input any, tools *taskengine.ToolsCall) (any, taskengine.DataType, error) {
	ordinal := extractOrdinal(input, tools)
	if ordinal <= 0 {
...
func (h *PlanManagerTools) skipStep(ctx context.Context, input any, tools *taskengine.ToolsCall) (any, taskengine.DataType, error) {
	ordinal := extractOrdinal(input, tools)
	if ordinal <= 0 {
...
func extractGoal(input any, tools *taskengine.ToolsCall) string {
	if tools != nil {
		if g, ok := tools.Args["goal"]; ok && g != "" {
			return g
		}
	}
...
func extractOrdinal(input any, tools *taskengine.ToolsCall) int {
	if tools != nil {
		if raw, ok := tools.Args["ordinal"]; ok && raw != "" {
			var n int
...
func (h *PlanManagerTools) Supports(_ context.Context) ([]string, error) {
	return []string{planManagerToolsName}, nil
}

// GetSchemasForSupportedTools implements taskengine.ToolsRepo.
func (h *PlanManagerTools) GetSchemasForSupportedTools(_ context.Context) (map[string]*openapi3.T, error) {
	...
	schema := &openapi3.T{
		OpenAPI: "3.1.0",
		Info:    &openapi3.Info{Title: "Plan Manager Tools", Description: "LLM-driven plan orchestration tools", Version: "1.0.0"},
	...
	return map[string]*openapi3.T{planManagerToolsName: schema}, nil
}

// GetToolsForToolsByName implements taskengine.ToolsRepo.
func (h *PlanManagerTools) GetToolsForToolsByName(_ context.Context, name string) ([]taskengine.Tool, error) {
	if name != planManagerToolsName {
		return nil, fmt.Errorf("plan_manager: unknown tools %q", name)
	}
...
var _ taskengine.ToolsRepo = (*PlanManagerTools)(nil)
