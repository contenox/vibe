...
func HostFromRepo(repo taskengine.ToolsRepo) HostToolCaller {
	...
	return HostFunc(func(ctx context.Context, provider, tool string, args map[string]any) (any, error) {
		out, _, err := repo.Exec(reenterGate(ctx), time.Now().UTC(), args, false, &taskengine.ToolsCall{
			Name:     provider,
			ToolName: tool,
		})
		if err != nil {
...
// reenterGate severs the outer tool call's identity so a script's nested host.tool call is gated on its own merits, never inheriting the script's own approval verdict or recorded gate result (both lookups short-circuit on an empty tool-call ID).
func reenterGate(ctx context.Context) context.Context {
	return context.WithValue(ctx, taskengine.ContextKeyToolCallID, "")
}

// policyDenyPrefix opens every refusal the HITL wrapper renders from a policy
// rule; unlike the two fixed sentences the rest of it varies per rule, so only
// the opening is matchable.
const policyDenyPrefix = "Denied by the active policy"

// IsDenyMessage reports whether a host tool result is the envelope's refusal rather than data; a tool returning one of these sentences verbatim is an accepted false positive.
func IsDenyMessage(v any) bool {
	...
	return strings.HasPrefix(s, policyDenyPrefix)
}
...
// selfAddressed reports whether name addresses this sandbox's own provider,
// under either the registered (scoped) name or the bare one.
func (s *sandbox) selfAddressed(name string) bool {
	return name == s.provider || name == ToolsProviderName
}
	...
	call := func(fc goja.FunctionCall) goja.Value {
		...
		refuseRecursion := func() {
			refuse(ErrRecursionRefused, fmt.Sprintf(
				...
				ErrRecursionRefused, label, echoArg(name), s.provider))
		}

		provider, tool, err := splitToolName(name)
		if err != nil {
			if s.selfAddressed(name) || name == ToolEval || strings.HasPrefix(name, ToolsProviderName+"_") {
				refuseRecursion()
			}
		...
		if hs.calls >= hs.maxCalls {
			refuse(ErrHostBudget, fmt.Sprintf(
				...
				ErrHostBudget, label, hs.maxCalls))
		}
		hs.calls++
...
func newHostState(maxCalls int) *hostState {
