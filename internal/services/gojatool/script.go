...
type Script struct {
	...
	// message uses.
	File string
	// Name is the declared tool name, registered unprefixed: the toolset is
	// the namespace, so the model addresses it as <toolset>.<Name>.
	Name string
...
func (s *sandbox) loadScripts(ctx context.Context, dir string, reserved []string) ([]*Script, error) {
	...
	for _, name := range reserved {
		if name != "" {
			taken[name] = "a built-in tool of this toolset"
		}
	}
	...
	for _, file := range files {
		...
		if owner, dup := taken[sc.Name]; dup {
			return nil, wrapRecoverable(ErrScriptLoad, "%s declares tool name %q, which is already %s. Tool names are unique within the %q toolset; rename one of them",
				file, sc.Name, owner, s.provider)
		}
...
func (s *sandbox) loadScript(ctx context.Context, file string) (*Script, error) {
	...
	// Load-time execution runs under the same deadline/stack/panic recovery as a real run; host.tool is refused here (see installHost). A chain-level policy cannot reach a load, which happens once at construction.
	res, err := s.run(ctx, execSpec{
		label:       label,
		lim:         s.base,
		deadline:    s.base.deadline,
		hostEnabled: false,
	...
	if res.Truncated {
		return nil, wrapRecoverable(ErrScriptLoad, "%s exports a `tool` descriptor larger than the %d-byte output cap. Keep name/description/schema small", file, s.base.outputCap)
	}
...
func (s *sandbox) validateDescriptor(file string, prog *goja.Program, desc toolDescriptor) (*Script, error) {
	...
	case strings.Contains(name, "."):
		return nil, wrapRecoverable(ErrScriptLoad, "%s: tool.name %s contains a dot. The toolset %q is already the namespace — the engine addresses this tool as %s.<name>, so the name itself must be a plain identifier",
			file, echoArg(name), s.provider, s.provider)
	...
	case strings.HasPrefix(name, ToolsProviderName+"_"):
		return nil, wrapRecoverable(ErrScriptLoad, "%s: tool.name %q uses the reserved %q prefix, which belongs to this toolset's built-in tools (%s). Pick a name that says what the script does",
	...
	declaredTools, toolsDeclared, err := s.normaliseDeclaredTools(file, desc.Tools)
	if err != nil {
		return nil, err
	}
	...
	if len(desc.DeadlineMS) > 0 && string(desc.DeadlineMS) != "null" {
		...
		deadline = s.base.clampDeadline(time.Duration(ms) * time.Millisecond)
	}
...
func (s *sandbox) normaliseDeclaredTools(file string, raw json.RawMessage) (names []string, declared bool, err error) {
...
func (s *sandbox) execScript(ctx context.Context, lim limits, sc *Script, args map[string]any) (*Result, error) {
	...
	return s.run(ctx, execSpec{
		label:       sc.Name,
		lim:         lim,
		deadline:    lim.clampDeadline(sc.Deadline),
		hostEnabled: true,
		reach:       sc.reach,
