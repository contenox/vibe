...
func TestUnit_Tools_SupportsNamesTheProviderAndItsTools(t *testing.T) {
	...
	want := []string{ScopedToolsProviderName, ToolEval, "echo_upper", "leaker"}
	...
	if ScopedToolsProviderName != "native-goja" {
		t.Fatalf("registered name = %q, want \"native-goja\"", ScopedToolsProviderName)
	}
	if ToolEval != "goja_eval" {
		t.Fatalf("eval tool = %q", ToolEval)
	}
	if ts.Name() != ScopedToolsProviderName {
		t.Fatalf("Name() = %q, want the registration key", ts.Name())
	}
}

// A caller that registers the toolset under a different key gets a toolset
// that answers to that key, since Exec and the schema lookup both arrive
// under the map key rather than under the package's own constant.
func TestUnit_Tools_ConfigNameOverridesTheRegisteredName(t *testing.T) {
	ts, err := New(Config{Name: "native-goja-review"})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	t.Cleanup(ts.Shutdown)

	if ts.Name() != "native-goja-review" {
		t.Fatalf("Name() = %q", ts.Name())
	}
	names, _ := ts.Supports(context.Background())
	if names[0] != "native-goja-review" {
		t.Fatalf("Supports()[0] = %q, want the configured name", names[0])
	}
	if _, err := ts.GetToolsForToolsByName(context.Background(), "native-goja-review"); err != nil {
		t.Fatalf("the configured name does not resolve the tool list: %v", err)
	}
	docs, err := ts.GetSchemasForSupportedTools(context.Background())
	if err != nil {
		t.Fatalf("GetSchemasForSupportedTools: %v", err)
	}
	if _, ok := docs["native-goja-review"]; !ok {
		t.Fatalf("the published document is not keyed by the registered name: %v", docs)
	}
}
...
func TestUnit_Tools_SchemaShape(t *testing.T) {
	...
	all, err := ts.GetToolsForToolsByName(ctx, ScopedToolsProviderName)
	if err != nil {
...
func TestUnit_Tools_PublishedSchemaMatchesToolDescriptors(t *testing.T) {
	...
	doc, ok := docs[ScopedToolsProviderName]
	if !ok {
		t.Fatalf("no document published under %q, got %v", ScopedToolsProviderName, docs)
	}
	...
	declared, err := ts.GetToolsForToolsByName(ctx, ScopedToolsProviderName)
	if err != nil {
		t.Fatalf("GetToolsForToolsByName: %v", err)
	}
...
func TestUnit_Tools_ExecDispatch(t *testing.T) {
	...
	t.Run("goja_eval", func(t *testing.T) {
		out, dt, err := ts.Exec(ctx, time.Now(), map[string]any{"code": `[1,2,3].reduce((a,b)=>a+b, 0)`}, false,
			&taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval})
		if err != nil {
			t.Fatalf("Exec: %v", err)
		}
	...
	t.Run("a script tool", func(t *testing.T) {
		out, dt, err := ts.Exec(ctx, time.Now(), map[string]any{"text": "quiet"}, false,
			&taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: "echo_upper"})
		if err != nil {
			t.Fatalf("Exec: %v", err)
		}
		if dt != taskengine.DataTypeJSON {
	...
	t.Run("args from the ToolsCall", func(t *testing.T) {
		out, _, err := ts.Exec(ctx, time.Now(), "chat history, not an args map", false,
			&taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval, Args: map[string]string{"code": `"from args"`}})
		if err != nil {
			t.Fatalf("Exec: %v", err)
		}
...
func TestUnit_Tools_ExecRefusals(t *testing.T) {
	...
	_, dt, err := ts.Exec(ctx, time.Now(), map[string]any{}, false,
		&taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: "goja_transpile"})
	if err == nil {
...
func TestUnit_Tools_EveryExecErrorCarriesASeverityMarker(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"unknown tool", map[string]any{}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: "nope"}},
		{"unknown argument", map[string]any{"code": `1`, "x": 1}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval}},
		{"missing code", map[string]any{}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval}},
		{"missing required", map[string]any{}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: "echo_upper"}},
		{"syntax error", map[string]any{"code": `if (`}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval}},
		{"a throw", map[string]any{"code": `throw new Error("x")`}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval}},
		{"a cycle", map[string]any{"code": `const a={};a.s=a;a`}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval}},
		{"a deadline", map[string]any{"code": `while(true){}`, "deadline_ms": 100}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval}},
		{"a refused recursion", map[string]any{"code": `host.tool("goja.goja_eval", {})`}, &taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval}},
	} {
		...
		go func(w int) {
			...
			for r := 0; r < rounds; r++ {
				out, _, err := ts.Exec(ctx, time.Now(), map[string]any{"tag": fmt.Sprintf("%d-%d", w, r)}, false,
					&taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: "leaker"})
				if err != nil {
...
func TestUnit_Tools_ShutdownStopsTheProvider(t *testing.T) {
	...
	if _, _, err := ts.Exec(ctx, time.Now(), map[string]any{"code": `1`}, false,
		&taskengine.ToolsCall{Name: ScopedToolsProviderName, ToolName: ToolEval}); err != nil {
