...
func TestUnit_Script_LoadAndExecute(t *testing.T) {
	...
	res, err := ts.sb.execScript(context.Background(), ts.sb.base, sc, map[string]any{"path": "notes.md"})
	if err != nil {
...
func TestUnit_Script_LoaderFailureMatrix(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			"the scoped provider name",
			`const tool = { name: "native-goja", description: "d", schema: { type: "object", properties: {} } }; function run(){}`,
			"is the provider name itself",
		},
		{
...
func TestUnit_Script_ExecutionIsBoundedLikeEval(t *testing.T) {
	...
	start := time.Now()
	_, err = ts.sb.execScript(context.Background(), ts.sb.base, ts.Scripts()[0], map[string]any{})
