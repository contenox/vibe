...
// GetToolsForToolsByName returns the toolset's tool list, or one tool by name; a script tool's description and schema are exactly what its file declares.
func (t *Toolset) GetToolsForToolsByName(_ context.Context, name string) ([]taskengine.Tool, error) {
	...
	if name == t.name || name == ToolsProviderName || name == "" {
		return all, nil
	}
...
func (t *Toolset) GetSchemasForSupportedTools(ctx context.Context) (map[string]*openapi3.T, error) {
	declared, err := t.GetToolsForToolsByName(ctx, t.name)
	if err != nil {
		return nil, err
	}
	...
	return map[string]*openapi3.T{t.name: schema}, nil
}
