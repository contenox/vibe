...
func eval(t *testing.T, sb *sandbox, code string, deadline time.Duration) (*Result, error) {
	t.Helper()
	return sb.runSource(context.Background(), ToolEval, code, sb.base, sb.base.clampDeadline(deadline))
}
...
func TestUnit_Sandbox_DocumentedLimits(t *testing.T) {
	...
	if DefaultMaxHostCalls != 256 {
		t.Errorf("DefaultMaxHostCalls = %d, want 256 (blueprint)", DefaultMaxHostCalls)
	}

	sb := newTestSandbox(t)
	if sb.base.deadline != DefaultDeadline || sb.base.outputCap != DefaultOutputCap || sb.maxStack != defaultMaxCallStack {
		t.Fatalf("Config{} defaults = %s/%d/%d", sb.base.deadline, sb.base.outputCap, sb.maxStack)
	}
	if sb.base.maxHostCalls != DefaultMaxHostCalls {
		t.Fatalf("Config{} host-call budget = %d", sb.base.maxHostCalls)
	}

	if got := sb.base.clampDeadline(time.Hour); got != MaxDeadline {
		t.Errorf("clampDeadline(1h) = %s, want the %s ceiling", got, MaxDeadline)
	}
	if got := sb.base.clampDeadline(time.Nanosecond); got != minDeadline {
		t.Errorf("clampDeadline(1ns) = %s, want the %s floor", got, minDeadline)
	}
	if got := sb.base.clampDeadline(0); got != DefaultDeadline {
		t.Errorf("clampDeadline(0) = %s, want the default", got)
	}

	over := newSandbox(Config{Deadline: time.Hour, MaxDeadline: 5 * time.Hour, OutputCap: 1 << 30, MaxHostCalls: 1 << 20})
	if over.base.maxDeadline != MaxDeadline || over.base.deadline > MaxDeadline || over.base.outputCap != maxOutputCap {
		t.Fatalf("out-of-range config not clamped: %s/%s/%d", over.base.deadline, over.base.maxDeadline, over.base.outputCap)
	}
	if over.base.maxHostCalls != maxHostCallsCeiling {
		t.Fatalf("out-of-range host-call budget not clamped: %d", over.base.maxHostCalls)
	}
}
...
func TestUnit_Sandbox_CallerCancellationInterrupts(t *testing.T) {
	...
	start := time.Now()
	_, err := sb.runSource(ctx, ToolEval, `while(true){}`, sb.base, 20*time.Second)
	if err == nil {
...
func TestUnit_Sandbox_ShutdownRefusesAndInterrupts(t *testing.T) {
	...
	go func() {
		_, err := ts.sb.runSource(context.Background(), ToolEval, `while(true){}`, ts.sb.base, 20*time.Second)
		done <- err
	}()
	time.Sleep(50 * time.Millisecond)
	...
	if _, err := ts.sb.runSource(context.Background(), ToolEval, `1`, ts.sb.base, time.Second); !errors.Is(err, ErrShutdown) {
