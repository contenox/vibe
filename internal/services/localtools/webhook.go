...
// WebToolsName is the registered toolset name an allowlist addresses; the
// native- prefix is a namespace, so a declared MCP source cannot mint the same
// key.
const WebToolsName = "native-web"

// WebCaller exposes per-verb HTTP tools (web_get, web_head, web_post, web_put, web_patch, web_delete) under the "native-web" namespace, each gated by tools_policies.native-web (host allow/deny, scheme allowlist, size limits, timeout, retry, redirects), tracked through libtracker, and — for mutating verbs — gated by HITL approval.
...
func NewWebCaller(tracker libtracker.ActivityTracker, options ...WebtoolsOption) taskengine.ToolsRepo {
	...
	wh := &WebCaller{
		client: newContainedWebClient(30 * time.Second),
		defaultHeaders: map[string]string{
...
func (h *WebCaller) validateURL(args map[string]string, raw string) (*url.URL, string, bool, error) {
	...
	if !containsFold(allowedSchemes, scheme) {
		return nil, fmt.Sprintf("native-web: scheme %q is not in allowed schemes %v", scheme, allowedSchemes), true, nil
	}
	host := u.Hostname()
	if host == "" {
		return nil, "native-web: URL must include a host", true, nil
	}
	// Link-local / cloud-metadata is a hard, non-opt-in block: 169.254.169.254 and
	// the rest of 169.254.0.0/16 (and IPv6 link-local) are never a legitimate tool
	// target, and this is the check every redirect hop re-runs. A name that resolves
	// to one is caught at dial time by denyLinkLocalDial.
	if ip := net.ParseIP(host); ip != nil && isBlockedEgressIP(ip) {
		return nil, fmt.Sprintf("native-web: host %q is a link-local / cloud-metadata address, which is never reachable", host), true, nil
	}
	...
	if hostMatches(host, denied) {
		return nil, fmt.Sprintf("native-web: host %q is denied by tools_policies.native-web._denied_hosts", host), true, nil
	}
	allowed := h.policyCSV(args, "_allowed_hosts", "")
	if len(allowed) > 0 && !hostMatches(host, allowed) {
		return nil, fmt.Sprintf("native-web: host %q is not in allowed hosts %v", host, allowed), true, nil
	}
	return u, "", false, nil
}
...
func (h *WebCaller) extractBody(input map[string]any, toolsCall *taskengine.ToolsCall, maxBytes int) (io.Reader, int, error) {
	...
	if maxBytes > 0 && len(raw) > maxBytes {
		return nil, 0, fmt.Errorf("request body is %d bytes (max %d); raise tools_policies.native-web._max_request_body_bytes or shrink the body", len(raw), maxBytes)
	}
...
func (h *WebCaller) doRequest(ctx context.Context, method, toolName string, input any, toolsCall *taskengine.ToolsCall) (any, taskengine.DataType, error) {
	...
	case http.MethodGet, http.MethodHead:
		if err := rejectUnknownArgs(WebToolsName+"."+toolName, dynArgs, "url", "headers", "query"); err != nil {
			return nil, taskengine.DataTypeAny, err
		}
	default:
		if err := rejectUnknownArgs(WebToolsName+"."+toolName, dynArgs, "url", "headers", "query", "body"); err != nil {
			return nil, taskengine.DataTypeAny, err
		}
	}
	...
	// Every redirect hop is re-validated against the same scheme/host/link-local
	// policy as the first URL: an allowed origin that 302s to a denied or
	// link-local host is refused before the hop is dialed.
	client.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if _, denial, denied, verr := h.validateURL(policy, req.URL.String()); verr != nil {
			return fmt.Errorf("%w: %v", errRedirectBlocked, verr)
		} else if denied {
			return fmt.Errorf("%w: %s", errRedirectBlocked, denial)
		}
		return nil
	}
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if doErr != nil {
			// A hop the policy refused, or a name that resolved to a link-local
			// address, is terminal — do not retry it into a storm.
			if errors.Is(doErr, errRedirectBlocked) || errors.Is(doErr, errEgressBlocked) {
				reportErr(doErr)
				return nil, taskengine.DataTypeAny, doErr
			}
	...
	if statusCode >= 200 && statusCode < 300 {
		...
		out := string(respBody)
		if truncated {
			out += fmt.Sprintf("\n\n[response truncated to %d bytes; raise tools_policies.native-web._max_response_bytes to read more]", maxRespBytes)
		}
		return out, taskengine.DataTypeString, nil
	}

	failure := fmt.Errorf("native-web %s %s: HTTP %d: %s", method, u.String(), statusCode, truncatedTail(respBody, 512))
	reportErr(failure)
...
func (h *WebCaller) Exec(ctx context.Context, _ time.Time, input any, _ bool, toolsCall *taskengine.ToolsCall) (any, taskengine.DataType, error) {
	if toolsCall == nil {
		return nil, taskengine.DataTypeAny, errors.New("native-web: tools_call required")
	}
	...
	default:
		return nil, taskengine.DataTypeAny, fmt.Errorf("native-web: unknown tool %q", toolName)
	}
}

// Supports advertises the scoped namespace alone: a bare verb name would be its
// own allowlist entry, separately addressable and surviving "!native-web", so
// the six verbs could no longer be granted or removed as one set.
func (h *WebCaller) Supports(_ context.Context) ([]string, error) {
	return []string{WebToolsName}, nil
}
...
func webTruncatedEnvelopeSchema() *openapi3.SchemaRef {
	return objectSchema("A JSON body that hit _max_response_bytes, wrapped so the cut is visible.",
		map[string]*openapi3.SchemaRef{
			...
			"_max_bytes":  intSchema("The cap that stopped it — tools_policies.native-web._max_response_bytes."),
...
func readVerbProps() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			"url":     map[string]any{"type": "string", "description": "Absolute URL to call. Scheme must be in tools_policies.native-web._allowed_schemes (default http,https). Host must pass _allowed_hosts / _denied_hosts policy."},
	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"body": map[string]any{
				...
				"description": "Request body. A string is sent as-is; any other JSON value is marshalled. Capped by tools_policies.native-web._max_request_body_bytes (default 256 KiB).",
			},
		},
		"required": []string{"url"},
	}
