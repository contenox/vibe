...
// TestUnit_JQTool_ContainmentRefusesTheControlPlane pins the other half of the
// vfs boundary: a path that stays inside the granted workspace but resolves at
// or under a registered control-plane directory is refused, and the refusal
// never carries the bytes it refused. This is the same process-global denylist
// local_fs and the shared server route their reads through, reached here through
// the same vfs.Contain seam — reconnected, not reinvented.
func TestUnit_JQTool_ContainmentRefusesTheControlPlane(t *testing.T) {
	// No t.Parallel: SetControlPlaneDenied is process-global; this runs in the
	// sequential phase and clears the denylist before any parallel test resumes.
	ws := t.TempDir()
	control := filepath.Join(ws, ".contenox")
	require.NoError(t, os.MkdirAll(control, 0o755))
	writeFixture(t, control, "policies.json", `{"token":"control-plane-secret"}`)
	writeFixture(t, ws, "chain.json", chainFixture)

	require.NoError(t, vfs.SetControlPlaneDenied(control))
	t.Cleanup(func() { _ = vfs.SetControlPlaneDenied() })

	tools := newTools(t, ws)

	// A path outside the control plane still reads, so the denial is the denylist
	// and not a broken root.
	res := mustExec(t, tools, map[string]any{"path": "chain.json", "filter": ".id"})
	assert.Equal(t, []string{`"acp-session"`}, values(res))

	for name, path := range map[string]string{
		"relative into the control plane": ".contenox/policies.json",
		"absolute into the control plane": filepath.Join(control, "policies.json"),
	} {
		name, path := name, path
		t.Run(name, func(t *testing.T) {
			_, err := exec(t, tools, map[string]any{"path": path, "filter": "."})
			require.Errorf(t, err, "%s must be refused", path)
			assert.Containsf(t, err.Error(), "control plane",
				"a control-plane refusal must name the boundary it hit, got: %v", err)
			assert.NotContains(t, err.Error(), "control-plane-secret",
				"a containment refusal must never carry the contents it refused")
		})
	}
}
...
func TestUnit_JQTool_ToolsRepoContract(t *testing.T) {
	...
	// Scoped name only: the bare tool is reachable through GetToolsForToolsByName
	// below, but it must not appear in the allowlist feed as its own entry, or
	// "!native-jq" would fail to remove it.
	assert.NotContains(t, supported, jqtool.ToolQuery)
