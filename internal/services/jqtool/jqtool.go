...
// and returns the emitted values. It is read-only, has no network access
// (input/inputs/import/include/$ENV are all compiled out), and every execution
// is deadline-bounded including recursion.
package jqtool
...
// "*" admits it like any other, "!native-jq" removes it. Policy addresses it as
// `tools_policies.native-jq`.
const ToolsProviderName = "native-jq"

// ToolQuery is the single tool this provider exposes. The name is the HITL
