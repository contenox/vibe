...
const queryDescription = "Run a jq program over ONE JSON or YAML document and get the values it emits. " +
	...
	"`path` reads the AGENT HOST's filesystem directly, not the editor's: unlike local_fs, whose reads route through the client, this toolset acts machine-locally by right of being named in the agent declaration. " +
	"So a path resolves against the host's allowed directory (tools_policies." + ToolsProviderName + "._allowed_dir when set), and a document only the editor can see must come in through `input`."
