...
// goEnvScrub filters a launched go toolchain's parent env by ScrubDenySecrets, the same posture resolvedSandboxEnv gives an agent shell.
var goEnvScrub = libsandbox.EnvScrub(libsandbox.ScrubDenySecrets, nil, nil)

// goInvocationEnv is the scrubbed env every `go` subprocess this package launches runs under; GOTOOLCHAIN=local is appended last so exec's dedup pins it and a workspace go.mod cannot trigger a toolchain download.
func goInvocationEnv() []string {
	env := os.Environ()
	if goEnvScrub != nil {
		env = goEnvScrub(env)
	}
