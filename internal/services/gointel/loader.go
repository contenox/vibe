...
func (ix *index) moduleRoot(ctx context.Context, dir string) (root, base string, err error) {
	...
	if err != nil {
		...
		if errors.Is(err, vfs.ErrControlPlane) {
			// Preserve vfs.ErrControlPlane so the runtime's own governing state is a branchable refusal, exactly like ErrEscape — the control-plane denylist is never a workspace.
			return "", "", wrapRecoverable(vfs.ErrControlPlane,
				"dir %s is inside the runtime control plane, which is never a workspace", echoArg(dir))
		}
