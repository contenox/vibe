...
// "*" admits it like any other, "!native-workspace" removes it.
const ToolsProviderName = "native-workspace"
...
const (
	...
	// because each hit carries its chunk text. topKMax is the default
	// ceiling only — a chain's tools policy moves it, see policy.go.
	topKDefault = 5
	topKMax     = 20
...
// ceiling is the effective one, which the tools policy may have moved.
func clampTopK(topK, ceiling int) int {
	if topK <= 0 {
		if topKDefault > ceiling {
			return ceiling
		}
		return topKDefault
	}
