...
// arguments. lim is the effective policy, so what the model is told the
// ceiling is and what a call actually enforces cannot disagree.
func searchProperties(lim limits) []searchProperty {
	return []searchProperty{
		...
		},
		{
			name:        "top_k",
			typ:         "integer",
			description: fmt.Sprintf("Maximum citations to return (default %d, ceiling %d). The result is also capped at ~%d tokens overall and says how many hits it withheld.", clampTopK(0, lim.topKMax), lim.topKMax, lim.resultTokenBudget),
		},
	}
}

// searchRequired renders the table's required set, in table order.
func searchRequired(lim limits) []string {
	var out []string
	for _, p := range searchProperties(lim) {
		if p.required {
...
func searchToolParameters(lim limits) map[string]any {
	props := make(map[string]any, len(searchProperties(lim)))
	for _, p := range searchProperties(lim) {
	...
	return map[string]any{
		"type":       "object",
		"properties": props,
		"required":   searchRequired(lim),
	}
}

// searchRequestSchema renders the same table as the published OpenAPI request
// schema.
func searchRequestSchema(lim limits) *openapi3.SchemaRef {
	props := make(map[string]*openapi3.SchemaRef, len(searchProperties(lim)))
	for _, p := range searchProperties(lim) {
	...
	allTools := []taskengine.Tool{
		{
			...
			Function: taskengine.FunctionTool{
				...
				Parameters:  searchToolParameters(lim),
			},
		},
	}
	...
	schema := &openapi3.T{
		...
		Components: &openapi3.Components{
			Schemas: map[string]*openapi3.SchemaRef{
				"WorkspaceSearchRequest":  searchRequestSchema(lim),
