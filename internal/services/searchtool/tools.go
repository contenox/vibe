...
type tools struct {
	q           Querier
	workspaceID string
	// name is the toolset key this repo is registered under, which is also
	// the key its tools-policy block and its HITL rules are written against.
	name string
}
...
func NewTools(q Querier, workspaceID string) taskengine.ToolsRepo {
	return &tools{q: q, workspaceID: workspaceID, name: ToolsProviderName}
}
...
func (h *tools) Exec(ctx context.Context, _ time.Time, input any, _ bool, call *taskengine.ToolsCall) (any, taskengine.DataType, error) {
	...
	case ToolSearch:
		...
		res, err := h.search(ctx, argString(args, "question"), topK, limitsFrom(ctx, h.name))
		if err != nil {
			return nil, taskengine.DataTypeAny, err
		}
...
func (h *tools) search(ctx context.Context, question string, topK int, lim limits) (*Result, error) {
	...
	hits, err := h.q.Query(ctx, h.workspaceID, question, clampTopK(topK, lim.topKMax))
	if err != nil {
	...
	res := renderHits(question, hits, lim)
...
func renderHits(question string, hits []workspaceindex.Hit, lim limits) *Result {
	res := &Result{
	...
	if withheld := res.Found - res.Shown; withheld > 0 {
		res.Note = appendNote(res.Note, fmt.Sprintf(
			...
			withheld, res.Found, lim.resultTokenBudget))
	}
	if stale > 0 {
		res.Note = appendNote(res.Note, fmt.Sprintf(
