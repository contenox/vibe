-- Postgres schema for Contenox Local. The SQLite file (schema_sqlite.sql) is the
-- source of truth for shape; keep the two diffable. TestUnit_Schema_Parity_*
-- enforces it: tables, columns, types, nullability, defaults, PRIMARY KEY and
-- UNIQUE membership, CHECKs, foreign keys and every index must agree. The
-- differences the two dialects legitimately have are listed as explicit
-- mappings in schema_parity_test.go (BLOB/BYTEA and so on); an unlisted one
...
-- Columns added after the initial release live in the migration section below,
...
-- estimate_row_count is the cheap pg_class estimate the store's Estimate*Count
-- methods call. reltuples is -1 for a table that has never been analyzed or
-- vacuumed (Postgres 14+), and the caller treats any negative answer as
-- "unknown" rather than a count.
...
-- The lexical half of hybrid retrieval, in its Postgres shape: the same three
-- columns the SQLite FTS5 mirror carries, so AppendWorkspaceChunks /
-- DeleteWorkspaceChunks* write both backends unchanged, plus a GIN index over
-- to_tsvector for the lexical narrowing. SearchWorkspaceChunks still speaks
-- MATCH/bm25 and has no Postgres path yet; that rewrite is not in this slice.
CREATE TABLE IF NOT EXISTS workspace_chunks_fts (
    text      TEXT NOT NULL,
    chunk_id  VARCHAR(255) NOT NULL,
    config_id VARCHAR(255) NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_workspace_chunks_fts_text
    ON workspace_chunks_fts USING GIN (to_tsvector('simple', text));
CREATE INDEX IF NOT EXISTS idx_workspace_chunks_fts_chunk
    ON workspace_chunks_fts (chunk_id);
CREATE INDEX IF NOT EXISTS idx_workspace_chunks_fts_config
    ON workspace_chunks_fts (config_id);

-- Incremental migrations — every ADD COLUMN carries IF NOT EXISTS, so an
-- already-upgraded database re-applies the whole section as a no-op.

-- remote_tools columns added after initial release
ALTER TABLE remote_tools ADD COLUMN IF NOT EXISTS headers             TEXT;
ALTER TABLE remote_tools ADD COLUMN IF NOT EXISTS properties         BYTEA;
ALTER TABLE remote_tools ADD COLUMN IF NOT EXISTS inject_params_json TEXT NOT NULL DEFAULT '{}';
ALTER TABLE remote_tools ADD COLUMN IF NOT EXISTS spec_url           TEXT;

-- mcp_servers columns added after initial release
ALTER TABLE mcp_servers ADD COLUMN IF NOT EXISTS headers_json            TEXT NOT NULL DEFAULT '{}';
ALTER TABLE mcp_servers ADD COLUMN IF NOT EXISTS inject_params_json      TEXT NOT NULL DEFAULT '{}';
...
ALTER TABLE remote_tools ADD COLUMN IF NOT EXISTS auth_flow_json TEXT;
...
INSERT INTO event_nid_seq (id, last_nid) VALUES (1, 0) ON CONFLICT (id) DO NOTHING;
...
-- The SQLite file ends with a copy-drop-rename rebuild of llm_backends, which
