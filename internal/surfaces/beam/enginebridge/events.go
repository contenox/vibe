...
// dialect.CommandValueDomains). An absent key means "no domain known" — treat
...
func ValueDomains(options []libacp.SessionConfigOption) map[string][]string {
	return dialect.CommandValueDomains(options)
}
...
func translate(n libacp.SessionNotification) Event {
	...
	case dialect.TerminalOutputUpdateKind:
		if raw, ok := metaEnvelope(u.Meta)[dialect.TerminalOutputMetaKey]; ok {
