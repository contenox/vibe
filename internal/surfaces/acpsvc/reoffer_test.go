...
func TestUnit_ReofferableAsk_RejectsWhatIsNotALiveQuestion(t *testing.T) {
	now := time.Now().UTC()
	live := func() *runtimetypes.HITLApproval {
	...
	require.True(t, reofferableAsk(live(), now))

	noDeadline := live()
	noDeadline.ExpiresAt = time.Time{}
	require.True(t, reofferableAsk(noDeadline, now), "an ask with no deadline never expires out of the inbox")

	require.False(t, reofferableAsk(nil, now))

	for name, mutate := range map[string]func(*runtimetypes.HITLApproval){
		"denied":   func(r *runtimetypes.HITLApproval) { r.State = runtimetypes.HITLApprovalDenied },
		"approved": func(r *runtimetypes.HITLApproval) { r.State = runtimetypes.HITLApprovalApproved },
		"expired":  func(r *runtimetypes.HITLApproval) { r.State = runtimetypes.HITLApprovalExpired },
		"past due": func(r *runtimetypes.HITLApproval) { r.ExpiresAt = now.Add(-time.Second) },
		"due now":  func(r *runtimetypes.HITLApproval) { r.ExpiresAt = now },
		"no id":    func(r *runtimetypes.HITLApproval) { r.ID = "" },
		"a question": func(r *runtimetypes.HITLApproval) {
			r.ToolsName, r.ToolName = hitlservice.AttentionToolsName, hitlservice.AttentionToolName
		},
	} {
		t.Run(name, func(t *testing.T) {
			row := live()
			mutate(row)
			require.False(t, reofferableAsk(row, now))
		})
	}
}
