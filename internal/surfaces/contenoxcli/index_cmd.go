...
func openWorkspaceIndex(ctx context.Context, cmd *cobra.Command, dir string) (io.Closer, *indexDeps, error) {
	...
	// A nil embedder still works: FTS5 answers exact identifiers without a semantic leg.
...
func bindWorkspaceSearch(tools map[string]taskengine.ToolsRepo, db libdb.DBManager, engine *Engine) {
	...
	// A nil embedder still yields a lexical-only querier; the tool is absent only when the toolset itself is.
	model := strings.TrimSpace(engine.EmbeddingModel.Name)
	var embedder workspaceindex.Embedder
	if model != "" {
		embedder = workspaceindex.NewLLMRepoEmbedder(engine.Models, model, engine.EmbeddingModel.Provider)
