...
// maxAttachmentBytes bounds a single --attach file, so a mistyped path fails
// with a clear message instead of ballooning the request.
const maxAttachmentBytes = 10 << 20 // 10 MiB

// loadImageAttachments reads each --attach path into the ImagePart shape the
// user message carries. Content type is sniffed from the bytes, and anything
// that does not sniff as an image is rejected.
